"use client";
import React from 'react'
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import Link from "next/link"
import { AutoComplete, type Option } from '@/components/common/Autocomplete';

export const Step1 = () => {
    const [value, setValue] = React.useState<Option>()
   
    return (
        <React.Fragment>
            <form>
                <h3 className="text mb-4 font-semibold">Choose your Linux Type</h3>
                <div className="grid gap-6 lg:grid-cols-2">
                    <div className="relative w-72 lg:w-96">
                        <input
                            className="peer hidden"
                            id="radio_1"
                            type="radio"
                            name="radio"
                            checked
                        />
                        <span className="peer-checked:border-blue-500 absolute right-4 top-1/2 box-content block h-3 w-3 -translate-y-1/2 rounded-full border-8 border-gray-300 bg-white" />
                        <label
                            className="peer-checked:border-2 peer-checked:border-blue-400 peer-checked:bg-blue-50 flex cursor-pointer select-none rounded-lg border border-gray-300 p-4 pr-20"
                            htmlFor="radio_1"
                        >
                            <svg
                                className="w-14"
                                version="1.1"
                                id="Layer_1"
                                xmlns="http://www.w3.org/2000/svg"
                                xmlnsXlink="http://www.w3.org/1999/xlink"
                                x="0px"
                                y="0px"
                                viewBox="0 0 512 512"
                                enableBackground="new 0 0 512 512"
                                xmlSpace="preserve"
                            >
                                <linearGradient
                                    id="SVGID_1_"
                                    gradientUnits="userSpaceOnUse"
                                    x1="221.3611"
                                    y1="347.9715"
                                    x2="222.498"
                                    y2="237.696"
                                    className="gradient-element"
                                >
                                    <stop
                                        offset={0}
                                        className="primary-color-gradient"
                                        style={{ stopColor: "#ABDCFF" }}
                                    />
                                    <stop
                                        offset={1}
                                        className="secondary-color-gradient"
                                        style={{ stopColor: "#0396FF" }}
                                    />
                                </linearGradient>
                                <path
                                    fill="url(#SVGID_1_)"
                                    d="M405.6,294.9c-9.3-6.3-20.6-10-32.7-10c-16.1,0-30.1,7.2-41.2,17.1c-15.4,13.7-13.5,41-15.8,52.2h-94	c-2.4-11.3,12.2-23.1-15.8-52.2c-10.3-10.8-25.1-17.1-41.2-17.1c-16.1,0-30.7,6.5-41.2,17.1c-10.5,10.5-17.1,25.1-17.1,41.2	c0,3.8,0.4,7.5,1,11H37.9v-23.1H61v-69.3c0,0,277.2,0,300.3,0C383.7,261.8,400.1,276.9,405.6,294.9z"
                                />
                                <path d="M372.9,408.4c-29,0-54.4-19.1-62.8-46.2h-82.3c-8.4,27.1-33.8,46.2-62.8,46.2c-36.3,0-65.8-29.5-65.8-65.8c0-4.4,3.6-8,8-8	s8,3.6,8,8c0,27.4,22.3,49.8,49.8,49.8c23.6,0,44.1-16.7,48.8-39.8c0.8-3.7,4-6.4,7.8-6.4h94.7c3.8,0,7.1,2.7,7.8,6.4	c4.7,23.1,25.2,39.8,48.8,39.8s44.1-16.7,48.8-39.8c0.8-3.7,4-6.4,7.8-6.4h12.7c21.1,0,26.7-17.4,26.7-26.7v-23.1	c0-68-19.8-116.4-59-143.9c-29.6-20.8-59.8-21.4-60.1-21.4h-231c-49.2,0-49.8,47.7-49.8,49.8v87.8c0,4.4-3.6,8-8,8s-8-3.6-8-8v-87.8	c0-5.4,1-24.2,13.3-41.1c8.2-11.3,24-24.7,52.5-24.7h231c1.4,0,135.1,2.1,135.1,181.3v23.1c0,20.7-14.9,42.7-42.7,42.7h-6.5	C427.3,389.3,401.8,408.4,372.9,408.4z M372.9,377.6c-19.3,0-35-15.7-35-35s15.7-35,35-35c4.4,0,8,3.6,8,8s-3.6,8-8,8	c-10.5,0-19,8.5-19,19s8.5,19,19,19s19-8.5,19-19c0-4.4,3.6-8,8-8s8,3.6,8,8C407.8,361.9,392.2,377.6,372.9,377.6z M164.9,377.6	c-4.4,0-8-3.6-8-8s3.6-8,8-8c10.5,0,19-8.5,19-19s-8.5-19-19-19s-19,8.5-19,19c0,4.4-3.6,8-8,8s-8-3.6-8-8c0-19.3,15.7-35,35-35	c19.3,0,35,15.7,35,35S184.2,377.6,164.9,377.6z M84.1,362.2H37.9c-4.4,0-8-3.6-8-8v-23.1c0-4.4,3.6-8,8-8H53v-24.7c0-4.4,3.6-8,8-8	s8,3.6,8,8v32.7c0,4.4-3.6,8-8,8H45.9v7.1h38.2c4.4,0,8,3.6,8,8S88.5,362.2,84.1,362.2z M429.5,339.1c-3.7,0-7.1-2.6-7.8-6.4	c-4.7-23.1-25.2-39.8-48.8-39.8s-44.1,16.7-48.8,39.8c-0.9,4.3-5.1,7.1-9.4,6.3c-4.3-0.9-7.1-5.1-6.3-9.4	c6.2-30.5,33.3-52.6,64.4-52.6s58.3,22.1,64.4,52.6c0.9,4.3-1.9,8.6-6.3,9.4C430.5,339,430,339.1,429.5,339.1z M221.5,339.1	c-3.7,0-7.1-2.6-7.8-6.4c-4.7-23.1-25.2-39.8-48.8-39.8c-4.4,0-8-3.6-8-8s3.6-8,8-8c31.2,0,58.3,22.1,64.4,52.6	c0.9,4.3-1.9,8.6-6.3,9.4C222.6,339,222.1,339.1,221.5,339.1z M447.5,246.7H119.9c-4.4,0-8-3.6-8-8v-92.4c0-4.4,3.6-8,8-8h212.4	c0,0,0,0,0.1,0l0.6,0c0.4,0,0.7,0,1.1,0.1c15.7,0.2,23.3,0.4,25.8,0.8c18.7,2.8,64.6,15.5,87.7,73.5c1.6,4.1-0.4,8.8-4.5,10.4	c-4.1,1.6-8.8-0.4-10.4-4.5c-20.1-50.3-59.3-61.2-75.2-63.6c-1.1-0.2-5.1-0.4-17.2-0.5v61.1c0,4.4-3.6,8-8,8s-8-3.6-8-8v-61.3h-94.2	v61.3c0,4.4-3.6,8-8,8s-8-3.6-8-8v-61.3h-86.2v76.4h319.6c4.4,0,8,3.6,8,8S451.9,246.7,447.5,246.7z" />
                                <path
                                    fill="#0396FF"
                                    className="secondary-color"
                                    d="M490.4,379.2v44.5H446v-44.5H490.4z M446,490.6h44.5v-44.5H446V490.6z M377.6,490.6h44.5v-44.5h-44.5V490.6z	"
                                />
                                <path d="M27.2,64.1c0-14.6,11.8-26.4,26.4-26.4c4.4,0,8,3.6,8,8s-3.6,8-8,8c-5.7,0-10.4,4.7-10.4,10.4s4.7,10.4,10.4,10.4	c4.4,0,8,3.6,8,8s-3.6,8-8,8C39,90.5,27.2,78.7,27.2,64.1z M137.3,450.4H63.5c-4.4,0-8,3.6-8,8s3.6,8,8,8h73.8c4.4,0,8-3.6,8-8	S141.7,450.4,137.3,450.4z M203.6,78.6h3.9c4.4,0,8-3.6,8-8s-3.6-8-8-8h-3.9v-3.9c0-4.4-3.6-8-8-8s-8,3.6-8,8v3.9h-3.9	c-4.4,0-8,3.6-8,8s3.6,8,8,8h3.9v3.9c0,4.4,3.6,8,8,8s8-3.6,8-8V78.6z M234,452h-3.9v-3.9c0-4.4-3.6-8-8-8s-8,3.6-8,8v3.9h-3.9	c-4.4,0-8,3.6-8,8s3.6,8,8,8h3.9v3.9c0,4.4,3.6,8,8,8s8-3.6,8-8V468h3.9c4.4,0,8-3.6,8-8S238.5,452,234,452z M464.3,101.4	c-4.2-4.2-11-4.2-15.2,0s-4.2,11,0,15.2c4.2,4.2,11,4.2,15.2,0C468.5,112.4,468.5,105.6,464.3,101.4z M372.2,66.2h-67.9	c-4.4,0-8,3.6-8,8s3.6,8,8,8h67.9c4.4,0,8-3.6,8-8S376.6,66.2,372.2,66.2z M109.5,268.7c0-4.4-3.6-8-8-8H16.9c-4.4,0-8,3.6-8,8	s3.6,8,8,8h84.6C106,276.7,109.5,273.2,109.5,268.7z" />
                            </svg>
                            <div className="ml-5">
                                <span className="mt-2 font-semibold">Micro VPS</span>
                                <p className="text-slate-500 text-sm leading-6">
                                    Lorem ipsum dolor, sit amet consectetur adipisicing.
                                </p>
                            </div>
                        </label>
                    </div>
                    <div className="relative w-72 lg:w-96">
                        <input className="peer hidden" id="radio_2" type="radio" name="radio" />
                        <span className="peer-checked:border-blue-500 absolute right-4 top-1/2 box-content block h-3 w-3 -translate-y-1/2 rounded-full border-8 border-gray-300 bg-white" />
                        <label
                            className="peer-checked:border-2 peer-checked:border-blue-400 peer-checked:bg-blue-50 flex cursor-pointer select-none rounded-lg border border-gray-300 p-4 pr-20"
                            htmlFor="radio_2"
                        >
                            <svg
                                className="w-14"
                                version="1.1"
                                id="Layer_1"
                                xmlns="http://www.w3.org/2000/svg"
                                xmlnsXlink="http://www.w3.org/1999/xlink"
                                x="0px"
                                y="0px"
                                viewBox="0 0 512 512"
                                enableBackground="new 0 0 512 512"
                                xmlSpace="preserve"
                            >
                                <linearGradient
                                    id="SVGID_1_"
                                    gradientUnits="userSpaceOnUse"
                                    x1="24.4969"
                                    y1="269.4882"
                                    x2="486.8723"
                                    y2="269.4882"
                                    className="gradient-element"
                                >
                                    <stop
                                        offset={0}
                                        className="primary-color-gradient"
                                        style={{ stopColor: "#ABDCFF" }}
                                    />
                                    <stop
                                        offset={1}
                                        className="secondary-color-gradient"
                                        style={{ stopColor: "#0396FF" }}
                                    />
                                </linearGradient>
                                <path
                                    fill="url(#SVGID_1_)"
                                    d="M70.1,294.4l-30-6.7c0,0-22.7-12.7-13.4-27.8c2.5-4,8.9-3.5,13.9-3.5H404c18.1,0,32.8-14.7,32.8-32.8	v-72.9c0-12.3,6.1-23.9,16.3-30.8l30.9-20.9l-17.9,56.2c0,0,11.3,24.9,15.4,42.2s-0.1,10.1,4.4,29.4c4.5,19.3-7.6,30.3-20.5,38.7	S331.5,298,331.5,298L405,428.2l-2.2,11.8h-41.3L194.4,301.4l-51.9-4.1L70.1,294.4z"
                                />
                                <path d="M325.6,227l-5.9,10.3h8.4c4.4,0,8,3.6,8,8s-3.6,8-8,8h-22.3c-2.9,0-5.5-1.5-6.9-4c-1.4-2.5-1.4-5.5,0-8l12.9-22.3	c2.2-3.8,7.1-5.1,10.9-2.9C326.5,218.3,327.8,223.2,325.6,227z M367.2,216.1c-3.8-2.2-8.7-0.9-10.9,2.9l-12.9,22.3	c-1.4,2.5-1.4,5.5,0,8c1.4,2.5,4.1,4,6.9,4h22.3c4.4,0,8-3.6,8-8s-3.6-8-8-8h-8.4l5.9-10.3C372.4,223.2,371,218.3,367.2,216.1z	 M278.1,216.1c-3.8-2.2-8.7-0.9-10.9,2.9l-12.9,22.3c-1.4,2.5-1.4,5.5,0,8c1.4,2.5,4.1,4,6.9,4h22.3c4.4,0,8-3.6,8-8s-3.6-8-8-8	h-8.4L281,227C283.2,223.2,281.9,218.3,278.1,216.1z M233.6,216.1c-3.8-2.2-8.7-0.9-10.9,2.9l-12.9,22.3c-1.4,2.5-1.4,5.5,0,8	c1.4,2.5,4.1,4,6.9,4H239c4.4,0,8-3.6,8-8s-3.6-8-8-8h-8.4l5.9-10.3C238.7,223.2,237.4,218.3,233.6,216.1z M189,216.1	c-3.8-2.2-8.7-0.9-10.9,2.9l-12.9,22.3c-1.4,2.5-1.4,5.5,0,8c1.4,2.5,4.1,4,6.9,4h22.3c4.4,0,8-3.6,8-8s-3.6-8-8-8H186l5.9-10.3	C194.1,223.2,192.8,218.3,189,216.1z M497.4,232.9c0.1,19.5-16.4,38.2-37.6,42.5c-30.3,6.2-64.3,15.2-94.3,23.2	c-24.6,6.5-45.9,12.1-58.2,14.3c-0.5,0.1-1,0.1-1.4,0.1c-3.8,0-7.2-2.7-7.9-6.6c-0.8-4.3,2.1-8.5,6.5-9.3c11.7-2.1,32.7-7.7,57-14.1	c30.2-8,64.5-17,95.2-23.4c13.8-2.8,24.9-14.8,24.9-26.8c-0.1-13.2-13.7-23.3-37.4-27.9c-4.3-0.8-7.2-5-6.3-9.4s5-7.2,9.4-6.3	c10.8,2.1,19.2,5,25.9,8.5l-18.8-38.2c-1.1-2.2-1.1-4.8,0-7.1l22.2-45.2l-71,48.1h28.1c4.4,0,8,3.6,8,8s-3.6,8-8,8h-62	c-1.2,1.1-3.7,5.7-3.8,13.7h4.9c4.4,0,8,3.6,8,8s-3.6,8-8,8H320c0,0,0,0-0.1,0H200.2c0,0,0,0-0.1,0h-72.5	c-27.2,0-39.5,8.6-48.3,21.9c6.2,10.1,13.3,14.1,24.3,14.1h46.2c4.4,0,8,3.6,8,8s-3.6,8-8,8h-46.2c-15.9,0-27.4-6-36.4-19.3	c-31,18.7-33.7,26.9-33.7,33.6c0,2.5,3.9,7.9,18.7,12.8c12.1,4,35.4,8.9,77.3,8.9H198c2,0,3.9,0.7,5.4,2.1l156.3,142.1	c3.2,2.9,5.6,4.3,10.5,4.3H399c0.5,0,0.9-0.3,1.2-0.7c0.2-0.4,0.2-0.9,0-1.3L333.5,319c-2.2-3.8-0.9-8.7,3-10.9	c3.8-2.2,8.7-0.9,10.9,3L414,427.8c3.1,5.3,3.1,11.9,0,17.3c-3.1,5.4-8.8,8.7-15,8.7h-28.7c-10.5,0-16.4-4.1-21.3-8.5l-154-140	h-65.4c-71.1,0-112-13.7-112-37.7c0-15,7.5-27.5,46.7-50.4c13.6-22.3,32.7-31.8,63.4-31.8h69.5L326.7,67.5	c4.8-4.4,10.8-8.5,21.3-8.5h28.8c6.2,0,11.9,3.3,15,8.7c3.1,5.3,3.1,12,0,17.3c0,0,0,0,0,0l-57.9,100.2h17.9	c0.2-17.2,8.2-29.7,19.1-29.7h17.8c0.2-5.1,2.6-9.7,6.7-12.5l76.7-52c2.3-1.6,4.9-2.4,7.7-2.4c4,0,7.9,1.8,10.6,5	c4.1,4.8,4.9,11.9,2.1,17.7l-22,44.7l22,44.7c1.8,3.6,0.6,8-2.7,10.2C496.1,219.4,497.4,227.9,497.4,232.9z M220.9,185.3h94.4	l62.5-108.2c0.2-0.4,0.2-0.9,0-1.4c-0.2-0.4-0.7-0.6-1.1-0.6h-28.8c-4.9,0-7.3,1.4-10.5,4.3L220.9,185.3z" />
                                <g>
                                    <path
                                        fill="#0396FF"
                                        className="secondary-color"
                                        d="M188.6,436.9v46.3h-22.5v-46.3H188.6z M76.3,436.9v46.3h22.5v-46.3H76.3z"
                                    />
                                    <path
                                        fill="#ABDCFF"
                                        className="primary-color"
                                        d="M143.2,436.9v46.3h-21.6v-46.3H143.2z M31.8,436.9v46.3h21.6v-46.3H31.8z"
                                    />
                                </g>
                                <path d="M506.8,344.6c0,4.4-3.6,8-8,8H412c-4.4,0-8-3.6-8-8s3.6-8,8-8h86.8C503.2,336.6,506.8,340.2,506.8,344.6z M445.7,316.5	c0-4.4-3.6-8-8-8h-57.4c-4.4,0-8,3.6-8,8s3.6,8,8,8h57.4C442.1,324.5,445.7,320.9,445.7,316.5z M173.1,131.9H86.3c-4.4,0-8,3.6-8,8	s3.6,8,8,8h86.8c4.4,0,8-3.6,8-8S177.5,131.9,173.1,131.9z M54.6,119.7H112c4.4,0,8-3.6,8-8s-3.6-8-8-8H54.6c-4.4,0-8,3.6-8,8	S50.2,119.7,54.6,119.7z M216.2,77.5h-3.9v-3.9c0-4.4-3.6-8-8-8s-8,3.6-8,8v3.9h-3.9c-4.4,0-8,3.6-8,8s3.6,8,8,8h3.9v3.9	c0,4.4,3.6,8,8,8s8-3.6,8-8v-3.9h3.9c4.4,0,8-3.6,8-8S220.6,77.5,216.2,77.5z M114.7,335.5l-14.1,1.1c-26.9,2.1-52.9-6.3-73.3-23.7	c-3.4-2.9-8.4-2.5-11.3,0.9c-2.9,3.4-2.5,8.4,0.9,11.3c21.2,18.2,47.8,27.9,75.6,27.9c3.1,0,6.2-0.1,9.3-0.4l14.1-1.1	c4.4-0.3,7.7-4.2,7.4-8.6C122.9,338.5,119.1,335.2,114.7,335.5z" />
                            </svg>
                            <div className="ml-5">
                                <span className="mt-2 font-semibold">Smart VPS</span>
                                <p className="text-slate-500 text-sm leading-6">
                                    Lorem ipsum dolor, sit amet consectetur adipisicing.
                                </p>
                            </div>
                        </label>
                    </div>
                </div>

                <div className='mt-8 flex gap-4'>
                    <div className="w-1/2 ">
                        <div>
                            <Label htmlFor="name">Name</Label>
                            <Input
                                id="name"
                                type="name"
                                placeholder="John Doe"
                                required
                            />
                        </div>
                        <div className="mt-2">
                            <Label htmlFor="email">Email</Label>
                            <Input
                                id="email"
                                type="email"
                                placeholder="m@example.com"
                                required
                            />
                        </div>
                    </div>
                    <div className="w-1/2 ">
                        <div className="gap-2">
                            <Label htmlFor="password">Password</Label>
                            <Input id="password" type="password" required />
                        </div>
                        <AutoComplete
                            options={[
                                {
                                    value: "postgres",
                                    label: "PostgresSQL",
                                },
                                {
                                    value: "sqlite",
                                    label: "Sqlite",
                                },
                                {
                                    value: "mysql",
                                    label: "MySQL",
                                },
                            ]}
                            emptyMessage="No resulsts."
                            placeholder="Find something"
                            isLoading={false}
                            onValueChange={setValue}
                            value={value}
                            disabled={false}
                        />
                    </div>


                </div>


                <Button type="submit" className="w-full mt-8 rounded-full" >
                    Get Started
                </Button>


            </form>
        </React.Fragment>
    )
}


